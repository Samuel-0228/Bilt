...
const SECRET_PATTERNS = [
  { name: 'API key', pattern: /ENV\s+\w*(API_KEY|APIKEY|API_SECRET)\w*\s*=\s*["']?(?!(\$\{|<|your_|changeme|placeholder|xxx))\S+/i },
  { name: 'password', pattern: /ENV\s+\w*(PASSWORD|PASSWD|DB_PASS)\w*\s*=\s*["']?(?!(\$\{|<|your_|changeme|placeholder|xxx))\S+/i },
  { name: 'token', pattern: /ENV\s+\w*(TOKEN|SECRET_KEY|PRIVATE_KEY|ACCESS_KEY)\w*\s*=\s*["']?(?!(\$\{|<|your_|changeme|placeholder|xxx))\S+/i },
  { name: 'connection string', pattern: /ENV\s+\w*(DATABASE_URL|REDIS_URL|MONGO_URI|CONNECTION_STRING)\w*\s*=\s*["']?(?!(\$\{|<|your_|changeme|placeholder|xxx))\S+/i },
];
