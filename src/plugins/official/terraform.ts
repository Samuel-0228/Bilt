...
import type { PluginManifest, PluginContext, PluginResult, ScanFinding } from '../../types/index.js';

// Patterns that match hardcoded secrets in Terraform files
const TF_SECRET_PATTERNS = [
  {
    name: 'AWS access key',
    pattern: /access_key\s*=\s*"(?!(\$\{|var\.|local\.|data\.))(AKIA[0-9A-Z]{16})"/i,
  },
  {
    name: 'AWS secret key',
    pattern: /secret_key\s*=\s*"(?!(\$\{|var\.|local\.|data\.))[A-Za-z0-9/+=]{40}"/,
  },
  {
    name: 'password',
    pattern: /password\s*=\s*"(?!(\$\{|var\.|local\.|data\.|<|your_|changeme|placeholder))[^"]{6,}"/i,
  },
  {
    name: 'token',
    pattern: /token\s*=\s*"(?!(\$\{|var\.|local\.|data\.|<|your_|changeme|placeholder))[^"]{10,}"/i,
  },
  {
    name: 'private key',
  ...
  {
    name: 'secret',
    pattern: /(?:client_secret|api_secret|secret_access_key)\s*=\s*"(?!(\$\{|var\.|local\.|data\.))[^"]{8,}"/i,
  },
  {
    name: 'connection string',
    pattern: /(?:connection_string|database_url)\s*=\s*"(?!(\$\{|var\.|local\.|data\.))[^"]+:\/\/[^"]+"/i,
  },
];
...
const plugin: PluginManifest = {
  ...
  version: '1.0.0',
  description: 'Scans Terraform files for hardcoded credentials and validates .gitignore for .tfvars.',
