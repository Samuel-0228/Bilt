...
import { detectFramework, checkClientExposedSecrets } from '../src/core/scan/framework.js';
...
describe('detectFramework', () => {
  ...
  it('should return undefined for unknown frameworks', async () => {
    ...
    try {
      ...
      if (framework) {
        expect(framework.clientExposedPrefixes.length).toBeGreaterThanOrEqual(0);
      }
    } finally {
      await fs.rm(tmpDir, { recursive: true, force: true });
    }
