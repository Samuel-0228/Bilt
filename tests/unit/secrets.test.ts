...
import {
  ...
  isHighEntropy,
} from "../../src/core/rules/entropy.js";
import { SECRET_RULES } from "../../src/core/rules/secret-rules.js";
import {
  PROVIDER_MAP,
  detectProvider,
} from "../../src/core/rules/providers.js";
...
describe("isHighEntropy", () => {
  it("should flag random-looking strings as high entropy", () => {
    ...
    expect(isHighEntropy("sk_" + "test_4eC39HqLyjWDarjtT1zdp7dc", 4.0)).toBe(
      true,
    );
  });
...
describe("SECRET_RULES", () => {
  ...
  it("should match AWS access key pattern", () => {
    const rule = SECRET_RULES.find(
      (r) => r.id.includes("aws") && r.name.toLowerCase().includes("access"),
    );
    if (rule) {
  ...
  it("should match Stripe live key pattern", () => {
    ...
    if (rule) {
      expect(rule.pattern.test("sk_" + "live_000000000000000000000000")).toBe(
        true,
      );
    }
  });
